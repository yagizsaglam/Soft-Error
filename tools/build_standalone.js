/* Builds chapter4_standalone.html — one self-contained file that opens by
   ...
   Everything the page fetches at runtime gets inlined:
     ...
     models/*.glb                   -> base64 -> loader.parseAsync()
     models/schedule.json           -> inline JS object
*/
const fs = require('fs');
...
html = html.replace(
  ...
  `  window.__SCHEDULE__   = ${read('models/schedule.json').toString('utf8')};\n` +
  `  window.__GLB_POINTS__ = "${b64('models/points.glb')}";\n` +
  `  window.__GLB_SCENE__  = "${b64('models/scene_web.glb')}";\n` +
  `  window.__GLB_FUN__    = "${b64('models/fun.glb')}";\n` +
...
const loadBlock = /Promise\.all\(\[\s*fetch\('models\/schedule\.json'\)[\s\S]*?loader\.loadAsync\('models\/fun\.glb'\)\s*\]\)/;
if (!loadBlock.test(html)) throw new Error('load block not found — chapter4.html shape changed');
html = html.replace(
  loadBlock,
  `Promise.all([\n` +
...
html = html.replace(
  '<title>Chapter 4 — Production</title>',
  '<title>Chapter 4 — Production (standalone)</title>'
);
